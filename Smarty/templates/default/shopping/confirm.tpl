...
<div id="undercolumn">
    <div id="undercolumn_shopping">
        <p class="block_name_list"><!--{$tpl_title|h}--><span>Order Confirm</span></p>        
        <p class="flow_area"><img src="<!--{$TPL_URLPATH}-->img/common/order_flow_4.jpg" alt="購入手続きの流れ" /></p>
        <p class="information">下記ご注文内容で送信してもよろしいでしょうか？<br />
            よろしければ、「<!--{if $use_module}-->次へ<!--{else}-->ご注文完了ページへ<!--{/if}-->」ボタンをクリックしてください。<br />
            <!--{if $arrForm.payment_id|h == 5}--><span style="color:red">※お支払い方法に「クレジットカード」を選択して頂いたお客様は次のステップにて「クレジットカード支払手続きへ」ボタンで<br />お支払い手続きを行って下さい。</span><!--{/if}-->
        </p>
        ...
        <input type="hidden" name="paymentid" value="<!--{$arrForm.payment_id}-->" />
        <div class="btn_area">
            <ul>
                <li>
        ...
        <table summary="ご注文内容確認">
            ...
            <tr>
                <th colspan="4" class="alignR" scope="row">
                    <!--{if $arrForm.mail_shipment_flg == true}-->
                    送料(メール便)
                    <!--{else}-->
