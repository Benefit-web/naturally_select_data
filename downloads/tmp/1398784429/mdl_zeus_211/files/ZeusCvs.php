 ...
 * @version $Id: ZeusCvs.php 22307 2013-09-25 05:03:18Z shimada $
 */

require_once(CLASS_REALDIR. 'SC_Session.php');

class ZeusCvs extends LC_Page_Ex {
    ...
    function init() {
        ...
        if ($this->isMobile) {
            $this->tpl_mainpage = realpath(dirname( __FILE__)) . "/tpl_cvs/zeus_cvs_mobile.tpl";
        } elseif ($this->isSPhone) {
            $this->tpl_mainpage = realpath(dirname( __FILE__)) . "/tpl_cvs/zeus_cvs_sphone.tpl";
        } else {
            $this->tpl_mainpage = realpath(dirname( __FILE__)) . "/tpl_cvs/zeus_cvs.tpl";
        }
    ...
    function process() {
        ...
        if( $paymentDbData['status'] != ORDER_PENDING || $paymentDbData['del_flg'] != 0 || $_SESSION['zeus_cvs_order_id'] == $_SESSION['order_id'] ){ // 決済処理中でなければエラーとする。
            SC_Utils_Ex::sfDispSiteError(PAGE_ERROR);
            exit;
        }
        $_SESSION['zeus_cvs_order_id'] = $_SESSION['order_id'];
    ...
    function setDispData($payment_id, $paymentDbData, $orderData ) {
        ...
        if ($this->isMobile) {
            $this->tpl_title = ZEUS_CVS_SITE_TITLE_MOBILE;
        }else{
            $this->tpl_title = ZEUS_CVS_SITE_TITLE;
        }
        $this->siteurl    = $paymentDbData['siteurl'];
        $this->sitestr    = $paymentDbData['sitestr'];
        $this->cvsLinkPointUrl = CVS_LINK_POINT_URL;
    }
