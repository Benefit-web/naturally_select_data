 ...
 * @version $Id: ZeusEdyRecv.php 22307 2013-09-25 05:03:18Z shimada $
 */

require_once CLASS_EX_REALDIR . 'page_extends/LC_Page_Ex.php';
require_once(MODULE_REALDIR . 'mdl_zeus_211/include.php');

class ZeusEdyRecv extends LC_Page_Ex {
    ...
    function ZeusEdyRecv() {
        $this->module_name = MDL_ZEUS_CODE;
        $this->pay_code = PAY_ZEUS_EDY;
    }
    ...
    function process() {
        ...
        GC_Utils::gfPrintLog('[ゼウス楽天Edy決済ステータス変更]処理開始');
        //------------------------------------------------------------------------------
        //-- リクエスト情報チェック
        //------------------------------------------------------------------------------
        $zeusResponse = array(
        ...
        GC_Utils::gfPrintLog('[ゼウス楽天Edy決済ステータス変更] ゼウスリクエストメソッド情報:'. $_SERVER['REQUEST_METHOD']. ' リクエスト情報:'. $requestData);
        ...
        if(count($mtbModuleData) == 0){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更]管理画面での設定が完了しておりません。');
        }
        ...
        if(empty($zeusResponse['order_no'])){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] ゼウスリクエストデータのオーダー番号が不正です。order_no:'. $zeusResponse['order_no']);
        }
        if($mtbModuleData[0]['memo01'] != $zeusResponse['clientip']){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] ゼウスリクエストデータのIPコードが不正です。clientip:'. $zeusResponse['clientip']);
        }
        if(empty($zeusResponse['sendid'])){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] ユニークキーが設定されてません。sendid:'. $zeusResponse['sendid']);
        }

        $dtbOrderData =  $this->getDtbOrderData($zeusResponse['sendid']);
        if(count($dtbOrderData) == 0){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] この注文情報は登録されておりません。注文番号:'.$zeusResponse['sendid']);
        }
        if($dtbOrderData['payment_id'] !== $mtbModuleData[0]['payment_id']){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] 楽天Edy決済データでは御座いません。注文番号:'.$zeusResponse['sendid']);
        }
        if(empty($zeusResponse['sendpoint'])){
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] フリーパラメータが設定されてません。sendpoint:'. $zeusResponse['sendpoint']);
        }
        if($this->sendPointCheck($zeusResponse['sendpoint'], $mtbModuleData[0]['memo02'], $zeusResponse['clientip'], $zeusResponse['sendid'])){  //EC-CUBE order_id
            $this->errorExit('[ゼウス楽天Edy決済ステータス変更] フリーパラメータが不正です。sendpoint:'. $zeusResponse['sendpoint']);
        }
        ...
        $updateMemo = "ゼウスオーダー番号：[".$zeusResponse['order_no']."]\n"
                    . "ゼウス注文番号：[".$zeusResponse['tracking_no']."]\n";
        $this->orderStatusUpdate($zeusResponse['sendid'], $zeusResponse['status'], $updateMemo);
        GC_Utils::gfPrintLog('[ゼウス楽天Edy決済ステータス変更]処理終了');

    }

    function errorExit($message) {
        GC_Utils::gfPrintLog($message);
        GC_Utils::gfPrintLog('[ゼウス楽天Edy決済ステータス変更]処理終了_受信データが不正です');
    ...
    function orderStatusUpdate($orderId, $zeusResponseStatus, $zeusResponseAll) {
        ...
        case ZEUS_EDY_PAID:          // 支払済
            $orderStatus = ORDER_PRE_END;    // ->入金済み
            break;

        case ZEUS_EDY_FAILED:        // 支払失敗
        ...
        if( $zeusResponseStatus == ZEUS_EDY_PAID ) $this->objPurchase->sendOrderMail( $orderId );
    }
}
